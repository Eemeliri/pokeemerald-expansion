...
static bool8 AllocPartyMenuBgGfx(void)
{
    ...
    case 1:
        if (!IsDma3ManagerBusyWithBgCopy())
        {
            ...
            DecompressDataWithHeaderWram(gPartyMenuBgScroll_Tilemap, sPartyScrollBgTilemapBuffer);
            LoadPalette(gPartyMenuBgScroll_Pal, BG_PLTT_ID(0), 7 * PLTT_SIZE_4BPP);
...
static void DisplayPartyPokemonBarDetailToFit(u8 windowId, const u8 *str, u8 color, const u8 *align)
{
    if (gPartyMenu.layout != PARTY_LAYOUT_SINGLE)
    {
...
static void DisplayPartyPokemonNickname(struct Pokemon *mon, struct PartyMenuBox *menuBox, u8 c)
{
    ...
    if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE)
    {
        ...
        DisplayPartyPokemonBarDetailToFit(menuBox->windowId, nickname, 0, menuBox->infoRects->dimensions);
    }
}
...
static void DisplayPartyPokemonLevel(u8 level, struct PartyMenuBox *menuBox)
{
    ...
    DisplayPartyPokemonBarDetailToFit(menuBox->windowId, gStringVar1, 0, &menuBox->infoRects->dimensions[4]);
}
