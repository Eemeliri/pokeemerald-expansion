...
static u32 GetTrainerPointsToGive(u16 trainerId)
{
    ...
    else
    {
        ...
        if (IsDoubleBattle())
            pointReward = 2 * trainerPoint;
        else
            pointReward = trainerPoint;
    }
...
static void Cmd_getmoneyreward(void)
{
    ...
    if (gBattleOutcome == B_OUTCOME_WON)
    {
        if (FlagGet(FLAG_POINT_BATTLE))
         {
             money = GetTrainerPointsToGive(TRAINER_BATTLE_PARAM.opponentA);
             if (gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS)
             money += GetTrainerPointsToGive(TRAINER_BATTLE_PARAM.opponentB);
             AddBattlePoints(money);
         }
         else
         {
