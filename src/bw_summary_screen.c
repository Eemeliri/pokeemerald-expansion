...
const u32 sSummaryScreen_Scroll_Gfx_BW[]                    = INCBIN_U32("graphics/summary_screen/bw/scroll_tiles.4bpp.smol");
...
static bool8 DecompressGraphics(void)
{
    ...
    case 1:
        ...
        {
            DecompressDataWithHeaderVram(sSummaryPage_Info_Tilemap_BW, sMonSummaryScreen->bg2TilemapBuffers[PSS_PAGE_INFO]);
            sMonSummaryScreen->switchCounter++;
        }
        break;
    case 2:
        DecompressDataWithHeaderVram(sSummaryPage_Skills_Tilemap_BW, sMonSummaryScreen->bg2TilemapBuffers[PSS_PAGE_SKILLS]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 3:
        DecompressDataWithHeaderVram(sSummaryPage_BattleMoves_Tilemap_BW, sMonSummaryScreen->bg2TilemapBuffers[PSS_PAGE_BATTLE_MOVES]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 4:
        DecompressDataWithHeaderVram(sSummaryPage_ContestMoves_Tilemap_BW, sMonSummaryScreen->bg2TilemapBuffers[PSS_PAGE_CONTEST_MOVES]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 5:
        DecompressDataWithHeaderVram(sSummaryEffect_Battle_Tilemap_BW, sMonSummaryScreen->bg1TilemapBuffers[PSS_EFFECT_BATTLE]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 6:
        DecompressDataWithHeaderVram(sSummaryEffect_Contest_Tilemap_BW, sMonSummaryScreen->bg1TilemapBuffers[PSS_EFFECT_CONTEST]);
        sMonSummaryScreen->switchCounter++;
        break;
    case 7:
        DecompressDataWithHeaderVram(sSummaryPage_ScrollBG_Tilemap_BW, sMonSummaryScreen->bg3TilemapBuffers);
        sMonSummaryScreen->switchCounter++;
        break;
    case 8:
...
static void DrawExperienceProgressBar(struct Pokemon *unused)
{
    ...
    if (summary->level < MAX_LEVEL && !FlagGet(FLAG_FRONTIER_LEVEL))
    {
...
static void PrintMonAbilityName(void)
{
    ...
    {
        PrintTextOnWindow(AddWindowFromTemplateList(sPageSkillsTemplate, PSS_DATA_WINDOW_SKILLS_ABILITY), gAbilitiesInfo[ability].name, 4, 2, 0, TEXT_COLOR_LIGHT_RED);
    }
    else
    {
...
static void PrintExpPointsNextLevel(void)
{
    ...
    if (sum->level < MAX_LEVEL && !FlagGet(FLAG_FRONTIER_LEVEL))
